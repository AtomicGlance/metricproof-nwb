...
| MetricProof-NWB | Preserve what was checked, against which exact file, using which validator versions/configuration. | A MetricProof evidence result for each validator, retaining validator-native fields where possible. | Yes: SHA-256, size, selected NWB metadata, producer identity, validator provenance, findings, and timestamp. |
